本文从「信道估计（channel estimation）」在 **SISO → MIMO** 的脉络出发，把网上一篇图解式博文中的**直觉**与 **LTE/OFDM 工程里常见的处理链条**对齐说明；文中对**规范结论**与**教学/经验表述**会分开标注。
...
- **SISO**：一个子载波上常写 $y = h\,x + n$。导频处 $x$、$y$ 已知时，希望求 $\hat{h}$ 使模型尽量贴合观测；最简情形会得到 $\hat{h} \approx y/x$，其背景是**最小二乘（LS）**，见 **§2.4**。有噪声时往往还要平滑、正则或维纳类后处理。  
- **MIMO（以 2 发 2 收为例）**：
...
### 2.4 最小二乘（LS）在说什么（与导频信道估计）

若你**还没接触过 LS**，可以先用两句中文记住它：

1. **「二乘」**：把误差取平方（放大大错、避免正负抵消）。  
2. **「最小」**：在一族候选解里，挑一个让**平方误差之和最小**的。

**单导频点、标量信道**：模型写为 $y \approx h\,x$，其中 $x$ 为已知参考，$y$ 为接收观测。若最小化 $|y - h x|^2$，当 $x \neq 0$ 时得到

$$
\hat{h} = \frac{y}{x}.
$$

这与 §2.2 中的 $\hat{h} \approx y/x$ 一致。工程实现里常写成「先乘共轭再按参考功率归一」：$\hat{h} = y\,x^*/|x|^2$；若 $|x|^2=1$ 则回到 $y/x$（与仓库里 CRS 上 $r\cdot\mathrm{conj}(s)$ 一类写法同族，**经验**）。

**多导频点**：若有多个 $y_k \approx h\,x_k$，LS 最小化 $\sum_k |y_k - h x_k|^2$，相当于用更多方程「平均」噪声；矩阵形式是超定线性系统的最小平方解（可用伪逆等），**思想仍是平方和最小**。

**和 MMSE / 后处理的分工**：LS **不显式**利用噪声功率；噪声大时 $\hat{h}$ 方差也大。因此实际链路常在 LS 之后再做 **频域平滑、变换域去噪、维纳型加权**（如 `v_crs_chnl_est`），或在检测阶段用 **MMSE** 显式引入 $\mathbf{R}_n$（见 §3.4、§4、§4.3）。

> [!note] 规范结论 vs 经验
> 「导频已知 + LS 估信道」是教材与实现中**极常见**的做法，属**经验算法族**；规范主要约束参考信号与接收机能力边界，**不**规定全网必须用某一种 LS 变体。

### 2.5 最小均方误差（MMSE）在说什么（与均衡/检测）

如果把接收端问题概括成一句话：**已知观测 $\mathbf{r}$，希望恢复发送符号 $\mathbf{s}$**。在线性接收机里常写作

$$
\hat{\mathbf{s}} = \mathbf{W}\mathbf{r}.
$$

**MMSE（Minimum Mean Square Error）** 的目标是选择 $\mathbf{W}$，使得均方误差最小：
...
它体现了一个直觉：**当噪声/干扰很大或很相关时，不要像 ZF 那样强行“逆信道”，而是用 $\mathbf{R}_n$ 做折中，减少噪声放大**。

> [!note] 在本仓库中的对应（经验）
> - 上行 2×2 合并 `Uplink/v_mimo2x2_combine.m` 里，按 RB 估计 $\mathbf{R}$（噪声协方差近似），逐子载波构造 $\mathbf{H}\mathbf{H}^H+\mathbf{R}$ 并求逆，再得到 $\mathbf{W}$ 去合并 $[r0;r1]$（见本文 §4.3）。

### 2.6 LS 与 MMSE：是不是“作用一样”？

它们都可用于“估计”，但**优化目标与使用信息不同**，因此不是简单的“场景不同而已”。

| 对比项 | LS（最小二乘） | MMSE（最小均方误差） |
|-------|-----------------|----------------------|
| 主要用途（在本文语境） | **导频处信道估计**的常见起步：由已知 $x$ 与观测 $y$ 拟合得到 $\hat{h}$ 或 $\hat{\mathbf{H}}$ | **均衡/检测**：由观测 $\mathbf{r}$ 恢复 $\mathbf{s}$，显式折中干扰与噪声放大 |
| 优化对象 | 残差平方：$\|y-hx\|^2$、$\|\mathbf{y}-\mathbf{H}\mathbf{x}\|^2$ | 估计误差的期望：$\mathbb{E}\|\mathbf{W}\mathbf{r}-\mathbf{s}\|^2$ |
| 需要的额外信息 | 不需要噪声统计（但可在后处理里间接利用） | 需要噪声/干扰的统计量（如 $\mathbf{R}_n$）与符号先验的假设 |
| 直觉 | “让模型尽量贴合观测” | “让估计值离真值平均更近”，在消干扰与抑噪之间折中 |
| 常见极限关系（直觉） | 当导频足够、噪声很小，LS 估计会更接近真实信道 | 当信噪比很高、且 $\mathbf{R}_n \to \mathbf{0}$ 时，MMSE 的权重形式会逐渐逼近 “逆信道/迫零（ZF）” 一类解；当噪声较大时则更偏向抑噪、避免放大 |

经验上：工程里经常是 **LS 得到稀疏/初始信道估计**，再做平滑/去噪得到更稳的 $\hat{\mathbf{H}}$，最后在检测阶段用 **MMSE**（或 IRC 等）把 $\hat{\mathbf{H}}$ 与 $\mathbf{R}_n$ 一起用起来（与本文 §3.4、§4、§4.3 的链条一致）。
...
在导频 RE 上，$\mathbf{x}$（或各端口 RS）与 $\mathbf{y}$ 已知/可测时，可在 LS 意义下解 $\mathbf{y} \approx \mathbf{H}\mathbf{x}$（标量直觉见 **§2.4**）。
...
下列 `.m` 路径均相对于该仓库的**根目录**（与本学习笔记所在目录无关；以你本机实际克隆路径打开即可）。
...
- **一页收齐**：原理与误区见 §1–§3、§5；**工程代码路径与数据流**见 **§4.3**；外链与延伸阅读见 §4.4、§6；可选插图与版权见 **§8**。

---

## 8. 插图与外链版权（可选）

- 正文中的 **Mermaid** 图为自包含示意图，无需额外文件即可阅读。  
- 若需补充 [博客园信道估计图解](https://www.cnblogs.com/louisanu/p/13046621.html) 中的插图，可自行截图后保存到本库 `DOCs/images/`，在相应小节引用。  
- [MathWorks](https://www.mathworks.com/help/lte/ug/channel-estimation.html)、[ShareTechnote](http://www.sharetechnote.com/html/Communication_ChannelEstimation.html) 等站点素材的版权归各站点所有，转载或对外分发时请遵守其条款。
