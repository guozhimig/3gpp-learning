...
>[!info] wbRssi 和 ibRssi
>在我们的系统log中，可以看到 wbRssi 和 ibRssi 的输出，
>wbRssi: 对应宽带的，指只经过了 RF 模拟滤波器，没有结果数字滤波器。
>问了一下学才，数字滤波器应该是按照我们系统的采样率和基带带宽做的比较陡峭的滤波，模拟滤波器就是 RC 滤波，斜率很大。
...
# 其他信道相关指标
我们的仪表测试中针对 PCT/RRM 测试，可以看到给的信道条件的方式是不同的。
PCT 往往会假设信道条件相对简单，而RRM则不会这么假设从而会给出具体的信道模型

## PCT 相关指标

![](assets/LTE%20学习笔记：一次搞懂%20RSRP、RSRQ、RSSI/file-20260729190859374.png)
...
| NocE-UTRA  <br>       | noise power spectral density for E-UTRA Cell<br>即 E-UTRA(LTE)小区的噪声功率谱<br>这种情况下我们就推算出 sinr = -80 - (-95) = 15<br>之所以我们能够直接这么运算是因为<br> |
|                       |                                                                                                                                      |
