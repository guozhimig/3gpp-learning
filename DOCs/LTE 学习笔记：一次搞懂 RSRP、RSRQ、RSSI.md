...
所以理论最大值就是 `-35.22 + 3 = -32.22`, 如果是纯空载，也就是只有参考信号时， 因为这里是双天线，port 0/1 参考信号各 2 个，总参考信号就是 4 个，满载时其他RE `12 - 4 = 8` 个 RE，每个RE有2个端口的信号叠加，总等效信号为 `4 + 8 * 2 = 20`, 比例： `(4/20) = 1/5`, 所以理论上的最小值就是 -32.22 -10*math.log(5,10) = 
