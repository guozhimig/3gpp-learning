...
> - `SINR`：不一定强绑定 RS，也可以基于其他已知序列（如 `CRS`、`DM-RS`、`CSI-RS`）估计；作为链路质量指标，通常同口径对比，更贴近解调条件与吞吐表现。
...
`RSSI` 是总接收功率，包括有用信号（RSRP）、噪声（noise）和干扰（interference）；在满载、等功率分布且采用对应统计口径的理想化前提下，`RSSI` 和 `RSRP` 的关系可近似写为：`RSSI ~= 12*N*RSRP`。
