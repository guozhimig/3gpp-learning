...
补充一个容易误解的点：`SINR` 不是“不依赖已知序列”，而是“不绑定单一参考序列”。  
在 LTE 实现里，常见可用于估计 `SINR` 的已知参考包括 `CRS`、`DM-RS`、`CSI-RS`（按测量对象和场景选择）。  
也就是说，`SINR` 估计通常仍建立在“已知参考 + 同口径资源统计”之上，只是它不像 `RSRP/RSRQ` 那样在定义层面强绑定某一种 RS 口径。

>[!tips] 工程直觉小结（便于记忆）
> - `RSRQ`：与 RS 口径强绑定，工程指标，偏移动性判决，口径有意设计为“混合统计”；
> - `SINR`：不一定强绑定 RS，链路质量指标，通常同口径对比，更贴近解调条件与吞吐表现。
