...
`SNR`（Signal to Noise Ratio）可写为：

`SNR = S / N`

其中 `S` 是有用信号功率，`N` 是噪声功率。与 `SINR` 相比，`SNR` 不包含外部干扰项。

在你参考的网页里，常见一句是“under full load and high SNR”。这里的 `high SNR` 是公式成立前提，不是公式中的变量。  
在这种理想化条件（噪声相对较低）下，才更容易使用下面的近似关系去建立直觉：

`RSRP(dBm) ~= RSSI(dBm) - 10*log10(12*N)`

可这样记：

- `SNR` 高：噪声影响小，近似关系更“干净”；
- `SNR` 低：噪声影响大，简单近似误差会更明显；
- 工程上最终仍要结合实测和配置，不应只靠单一近似式判断。
