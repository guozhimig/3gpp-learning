...
所以我们从UE看到的RSSI 为 `-34.3 - 3 =-37.3`, 理论最大值为 `-32.22 -（-60 - （-63）） = -35.22`, 因为仪表这个是单port的值，从UE的角度看， 它看到的是所有端口的总接收功率，所以就是, 
仪表理论值为满载，如果是纯空载，也就是只有参考信号时， 因为这里是双天线，port 0/1 参考信号各 2 个，总参考信号就是 4 个，满载时其他RE 12 - 4 =8 个 RE，每个有2个端口的信号叠加，
总等效信号为 `4 + 8 * 2 = 20`, 比例： `(4/20) = 1/5`
